...
\begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Images/FlightDynamics}
...
\begin{itemize}
\item I would like to thank Prof. Cunningham for his support with each of the volumes of the Flight Dynamics Bible.
\end{itemize}
...
In this chapter we cover a brief history of the early years of energy mechanics. I will mostly cover the main figures who are instrumental to our study of energy mechanics. These main figures are Isaac Newton, Pierre de Fermat, Johann Bernoulli, Leonhard Euler,  Joseph Louis-Lagrange, and William Hamilton.
...
Fermat is famous in mathematics for \textit{Fermat's Last Theorem}, which remained unsolved for 358 years. A solution involving elliptic curves, which you can find an in depth discussion in \cite{hellegouarch_invitation_2001} (this is entirely irrelevant to our work here). His contributions here, however, relate to the so-called \textit{Fermat's Principle of Least Time}. Fermat's biggest insight was the understanding that 'nature operates by means and ways that are easiest and fastest'. Although this way seem a simple statement, it will underlie the physical principle at the root of the calculus of variations.

Fermat's Principle of Least Time involves finding a light path such that the time is minimized. \footnote{It can be shown that this is not exactly true, as the only condition required for sufficiency in this problem is that the changes to the variation are of the second-order. Fermat's condition is simply one such case of this.}Fermat's set up is as shown in \ref{fig:least time ray}.
...
%Shape: Rectangle [id:dp6708114455602667] 
\draw  [color={rgb, 255:red, 126; green, 219; blue, 222 }  ,draw opacity=0.51 ][fill={rgb, 255:red, 126; green, 219; blue, 222 }  ,fill opacity=0.51 ] (110,160) -- (550,160) -- (550,280) -- (110,280) -- cycle ;
%Straight Lines [id:da6428685375335336] 
\draw    (130,90) -- (350,160) ;
\draw [shift={(130,90)}, rotate = 17.65] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]      (0, 0) circle [x radius= 3.35, y radius= 3.35]   ;
%Straight Lines [id:da7310900658578104] 
\draw    (350,160) -- (410,240) ;
\draw [shift={(410,240)}, rotate = 53.13] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]      (0, 0) circle [x radius= 3.35, y radius= 3.35]   ;
%Shape: Arc [id:dp45852649988194416] 
\draw  [draw opacity=0] (236.39,123.09) .. controls (252.29,76.98) and (297.15,43.75) .. (350,43.75) -- (350,160) -- cycle ; \draw [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,draw opacity=1 ]   (237.64,119.65) .. controls (254.54,75.34) and (298.47,43.75) .. (350,43.75) ; \draw [shift={(350,43.75)}, rotate = 180] [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,draw opacity=1 ][line width=0.75]    (0,5.03) -- (0,-5.03)   ; \draw [shift={(236.39,123.09)}, rotate = 289.02] [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ][line width=0.08]  [draw opacity=0] (8.04,-3.86) -- (0,0) -- (8.04,3.86) -- cycle    ;
%Shape: Arc [id:dp21512964060589046] 
\draw  [draw opacity=0] (382.69,203.39) .. controls (373.53,209.9) and (362.23,213.75) .. (350,213.75) -- (350,160) -- cycle ; \draw [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,draw opacity=1 ]   (380.17,205.08) .. controls (371.49,210.56) and (361.13,213.75) .. (350,213.75) ; \draw [shift={(350,213.75)}, rotate = 360] [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,draw opacity=1 ][line width=0.75]    (0,5.03) -- (0,-5.03)   ; \draw [shift={(382.69,203.39)}, rotate = 144.58] [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ][line width=0.08]  [draw opacity=0] (8.04,-3.86) -- (0,0) -- (8.04,3.86) -- cycle    ;
%Straight Lines [id:da11658348664865648] 
\draw [color={rgb, 255:red, 155; green, 155; blue, 155 }  ,draw opacity=1 ] [dash pattern={on 4.5pt off 4.5pt}]  (350,40) -- (350,280) ;

% Text Node
\draw (251,52.4) node [anchor=north west][inner sep=0.75pt]    {$\theta _{i}$};
% Text Node
\draw (361,220.4) node [anchor=north west][inner sep=0.75pt]    {$\theta _{r}$};
...
I do not delve into the proof by Fermat, but it involves a geometric proof using the Law of Cosines and finding the minimization by setting the derivative of the function of the length equal to zero $f'(0)=0$. This is a similar methodology we will see more rigorously defined in the calculus of variations. Fermat's result, in the modern language, is often referred to as \textit{Snell's Law}, which gives $n_i\sin\theta_i=n_r\sin\theta_r$. We will keep this result in mind, as we will see it again when we solve the problem of the \textit{Brachistichrone}.

\section{Principia (1687)}

It is in \textit{Principia} that we find the first evidences of Newton's contributions to the field of energy mechanics. It is Newton's work in the \textit{Principia} that most agree the first real works of the the calculus of variations are found.

In 1685, Newton formulated the \textit{minimal resistance problem}. In essense, this problem involves finding a solid of revolution for which the object produces a minimal drag. In Newton's time, no solidified theories of fluid dynamics had been formulated, so Newton suggested a fluid that is iso
...
We notice that with the \gls{Lagrangian} approach, we did not need to have an understanding of the vector directions, frame translations, or \gls{bke} to find our \gls{eom}. This, in addition to the flexibility of the generalized coordinates, is the real power of this approach.
 ...
 We have so far seen that Hamilton's principle is useful for solving these problems. Now, with our background for generalized coordinates built up, we can explore a derivation of the action which will explain the presence of the $T-U$ term and the $T+U$ term in the Hamiltonian.

The approach described here comes mostly from the work in \cite{carcassi_geometric_2023}. I have simplified some of the details of this approach to make this section more simple for the reader, but I recommend reading this paper for a deeper understanding of the subject.
