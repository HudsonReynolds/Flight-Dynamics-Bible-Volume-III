...
\section{Lie Groups}
Before we get to the formal definition of a group, it is helpful to have a mental picture of what a \gls{lie group} is in essence, which is shown in \autoref{fig:Lie Group}:
\begin{figure}[ht]
    \centering
...
\end{tikzpicture}
    ...
    \label{fig:Lie Group}
\end{figure}
This demonstrates the important property of the \gls{lie group} - it is both a group and a manifold. The blue `orb' denoted $G$ is representative of a smooth manifold in $\mathbb{R}^n$. We can think of the manifold in this example as the surface of a 2D ellipsoid on which all the members of the group must live. We can see three points defined $g_1$, $g_2$, and $g_1\cdot g_2$. In this example, $g_1\cdot g_2$ is simply the multiplication of $g_1$ and $g_2$.
...
\subsection{The Symmetries of Groups}
So, we have shown that we can make groups out of the elements of the number line. Previously, we have stated that groups contain information about symmetries. These symmetries may be hard to see in the example of the number line. In that case, it is helpful to give a pictorial representation instead. To do so, we will construct an equilateral triangle.
...
$q=q_0+iq_q+jq_2+kq_3 ,\quad q_0,q_1,q_2,q_3\in\mathbb{R}$
    ...
    We can again show all of the properties of the \gls{lie group} hold for the quaternion group.
\end{example}
...
Luckily, it is possible to represent almost any group using a matrix! This representation is typically denoted as $\rho$. We can then write the representation:
...
Interested readers can look more into \autoref{thm:cayley} and \autoref{thm:Ado}, which describe the details behind why this is true. These theorems will not be used elsewhere in this text, they are simply shown for completeness.
...
We have previously used representation theory to show why it is important to study matrix multiplication groups. To define matrix groups under multiplication, we need to again satisfy all four criteria of groups. For matrices, there are some subtleties that are worth covering. 
...
Starting with closure, we can write:
$$AB \in GL $$
Without additional information about $A$, $B$, and $GL$, it is hard to define additional properties at this point. 
...
and noticing that the identity matrix takes the form of Kronecker delta $\delta_{ij}$ such that $\delta_{ij}=1$ when $i=j$ and is zero otherwise. So, we have shown that the matrix $\mathbb{I}(n)$ must be an element of the group $G(n)$.
\end{proof}
...
\subsubsection{The role of the determinant}\label{sec:role of determinant}
...
Part of the answer lies in the fact that the \gls{lie group} must have a smooth manifold. This means that the spatial derivatives at a point must exist, because if it does not the curve may have a sharp discontinuity.
...
This is all that we will say about manifold structure for now. When studying  matrix \gls{lie group}, there are some special conditions that are required to show that they are manifolds, but we will only discuss those that are well behaved here. The interested reader can look into the cited sources for more information.
...
\begin{gather}
    ...
    bc+d^2=-1
\end{gather}
We also know that $|i|=1$, which gives us another condition. For a matrix, the analogue of the magnitude is the determinant, the reasoning for which I describe briefly in \autoref{sec:role of determinant}. This gives us $\det A=1$, yielding a final equation, $ad-bc=1$.
...
\begin{example}[Infinitesimal Rotations]
    ...
    In fact, your head is another good example! Most of our necks are not especially flexible (to rotate in the directions we need, at least). However we naturally associate motions of `
    ...
    \begin{figure}[H]
        ...
        \label{fig:large rot}
    \end{figure}
    As we can see, our monkey, Susanne, has ended up in very different rotations in either case, even though we performed the same rotation operations. Of course, if you have read Volume II, this is not at all an unexpected result. 

However, something strange happens if we consider rotations that are small in magnitude. This is best shown visually, which we will do in 
...
\begin{example}[Matrix Differential Equations]
    ...
    Generally, in control theory, we may denote $e^{At}$ (or $e^{A\Delta t}$, they are practically the same) as the \textit{state transition matrix}, which is sometimes denoted $\Phi$. This state transition matrix

\end{example}

% talk about lie theory non-linear vs diff eq linear
...
\end{theorem}
Although we have discussed continuous symmetries quite a bit, it is worth showing a few examples to solidify the meaning of this theorem.
...
\begin{gather}
    ...
    \end{bmatrix}
\end{gather}
We can deconstruct the meaning of the line to be: The special linear group is a group, such that set $\psi$, which is a subset of the general linear group $GL(2n,\mathbb{R})$, satisfies the property that $\psi^T\Omega\psi=\Omega$.
...
The subject of differential geometry was lightly touched upon in \autoref{ex:geodesic}, where we considered the shortest path on a spherical surface. Differential geometry is a field of study that is concerned with the study of smooth manifolds - which essentially means that it is an object which `looks' like a vector space when viewed closely. This is, of course, a loose definition that is not rigorous. In \autoref{ex:geodesic}, we showed this property on the sphere, 
...
So, this is a field of study that touches upon many of the mathematic concepts that we encounter in the study of dynamics.
...
We can take the analogy of walking through your house to demonstrate the key difference with the closed-loop system. Now, you have your eyes (the sensor) to aid you in walking through your house. This makes it much easier and you can account for disturbance events now. We can thus see that closed-loop systems are preferable in most applications. As a result, the rest of this part will put its emphasis on closed-loop system design.

\section{Types of Systems}

Generally, we can classify our systems

\subsubsection{Linear Systems}
The next level of distinction is more mathematical in nature. 
...
$$De^{ax}=ae^{ax}$$
So we have our eigenfunction, $e^{ax}$, which produces an eigenvalue $a$. Now, we want to use this useful property on equations that are not strictly exponentials. We can do so by \textit{projection} of these functions on to the exponential functions. How can we project functions on to each other? That is the function of the inner product, which we discuss next.
...
\end{gather}
The Laplace Transform will prove extraordinarily useful for analyzing LTI systems
