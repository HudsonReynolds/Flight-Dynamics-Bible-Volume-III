...
To define matrix groups, we need to again satisfy all of the four criteria. For matrices, we do this in a slightly different way that is worth covering here.
...
To describe a rotation mathematically, we first need to understand what rotations do. Aside from the obvious fact that rotations `rotate', what properties are inherit no matter what space we rotate in (although 2D and 3D are the most common for us).
...
\begin{enumerate}
    ...
    \item Lastly, and likely the most overlooked, rotations preserve the orientations of objects. If we have
...
\end{enumerate}
The first statement implies that $R$ must be square (Invertible Matrix Theorem). The second condition that $R^{-1}=R^T$ is a special condition which implies that the matrix is \textit{orthogonal}.
