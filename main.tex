...
\begin{itemize}
\item I would like to give my deepest appreciation and thanks to Professor Cunningham for his support with each of the volumes of the Flight Dynamics Bible and his support in helping Preston and I. I would also like to thank everyone at JHU APL who has helped me learn so much during my time there in the Summer of 2025. Their support and knowledge has been invaluable and has helped me in innumerable ways. I want to give special thanks to my mentors, Brian Caskey and Brandon Gray, who were especially helpful.
\end{itemize}
...
\section{Motivation and Background}\label{sec:motivationa and background lie}
\begin{chapquote}{Mark A. Armstrong}
    ``Numbers measure size, groups measure symmetry''
\end{chapquote}
Before we dive into the mathematical foundations of Lie theory, we should first examine why it is of importance to us. The quote above is a great primer and gives the essence of Lie theory. Lie theory is concerned with the inherit symmetries of mathematical objects. More specifically, the theory is concerned with so-called \textit{continuous symmetries}.

For us, this is especially useful in our study of dynamic systems. Often, we do have symmetries in our problems. There are those symmetries that are obvious - such as axial symmetry and reflectional symmetry of an object. There are deeper, more fundamental symmetries though, such as the symmetry between the rotational invariance of the Lagrangian and conservation of angular momentum (we will touch on this more in \autoref{Noether's Theorem}).

Such was the background under which Lie theory was developed. Originally, Sophus Lie (shown in \autoref{fig:Lie}) developed the theory in order to understand the continuous symmetries of differential equations. However, the theory proved to be much more useful in describing other symmetries, such as the rotations and conservation equations which we are familiar with in dynamics.
...
The other big part of Lie theory is that we can describe the attributes of our system at a point using \textit{algebras}. By thinking about our system as a manifold, we are able to describe coordinates for a certain location. Then, we can do something akin to a derivative - looking at the space `tangent' to the manifold at that point. This `tangent space' is what we call the \textit{Lie algebra}.

Lie algebras are useful because the tangent space is flat. Because it is flat, typical rules of geometry and calculus can be used, making analysis much easier.

The last important thing to mention is that we can convert between the Lie group and the Lie algebra using something called an \textit{exponential map}. For some groups and operations, this exponential map is simply an exponential function. Other times, however, we need a different way to describe these exponential transformations. This will be discussed later.

The most important thing to realize is that we can relate a complex non-linear system (represented in Lie theory by a group on a manifold) locally with a linearized version of the system, which we essentially 'project' onto the system via the exponential map.

\section{Lie Groups}

\subsection{The Axioms of Groups}
As mentioned before, the defining feature of the Lie group is that it is both a group and a manifold. We must first define the attributes of a group if we are to formally define this concept.
...
\begin{example}[The Real Numbers]
    ...
    Thus, we have shown that the integers are closed under addition. The reader should try this reasoning to show why the integers are not a group under multiplication. Which axiom is violated?
\end{example}
...
To define matrix groups under multiplication, we need to again satisfy all four criteria of groups. For matrices, there are some subtleties that are worth covering. 

There are a few types of matrix multiplication groups that are important for our purposes. Here, I will cover 4 of the most important ones to our study, starting with the most broad and adding constraints until we reach the most specific. Here, we will we cover $GL(n)$ and $SL(n)$.$O(n)$, $SO(n)$, $U(n)$, and $SU(n)$ will be covered later. We will go through these in this order, starting with $GL(n)$, deriving it from the fundamental axioms of groups.
...
The property of associativity also does not help much, since all $n{\times} n$ matrices must satisfy associativity by their nature:
$$(AB)C=A(BC)$$
...
and noticing that the identity matrix takes the form of Kronecker delta $\delta_{ij}$ such that $\delta_{ij}=1$ when $i=j$ and is zero otherwise. So, we have shown that the matrix $I(n)$ must be an element of the group $G(n)$.
\end{proof}
...
\subsubsection{The role of the determinant}
...
The other important defining aspect of Lie groups is that they are manifolds. More specifically, a Lie group must be a \textit{differentiable manifold}. 
...
To apply the rules of calculus, such as taking a derivative, it is necessary that we have a vector space. 

\section{Lie Algebras}
As before, I will start with the formal definition and then break down the meaning. Before going straight into Lie algebras, it is best to define what an \textit{algebra} is:
...
\end{definition}
We have seen most of the elements of this definition before when discussing Lie groups. The notable feature here is that two inputs from $\mathfrak{g}$, when multiplied, are still in $\mathfrak{g}$. This is a feature that we call \textit{bilinearity}. We may denote this in standard notation as $\mathfrak{g}\times \mathfrak{g}\rightarrow \mathfrak{g}$.
...
The particulars of algebras beyond this are not of great concern to us. Instead, we will be concerned with the specific properties of Lie algebras. The Lie algebra is similar to an algebra, but has some additional properties. Notably, we define an operation called the \textit{Lie bracket}, which we denote $[\cdot,\cdot]$. For a lie algebra, we must then have the following properties for the Lie bracket.
\begin{itemize}
    \item It is bilinear
    \item It is skew-symmetric, meaning that $[x,y]=-[y,x]$ for all $x,y$.
    \item It satisfies the Jacobi Identity, which states that: $[x,[y,z]]+[y,[z,x]]+[z,[x,y]]=0$.
\end{itemize}

Right now, this is a very vague definition which probably has no meaning to the reader. Let us consider the cross product as an example familiar to the reader:

\begin{example}[The cross product as a Lie algebra]
    We can start with the properties that we assumed for the Lie algebra. Taking the Lie bracket operation to be the cross product, that is: $[x,y]=x\times y$, it is easy to show that all of the properties are satisfied.
...
\begin{equation}
    \underset{\leftharpoondown}{\stackrel{\rightharpoonup}{xyzxy}}
\end{equation}
...
Since any vector crossed with itself is 0, the sum very obviously goes to zero. And so, we have shown that the cross product
\end{example}

\section{Relating Lie Groups and Algebras}
In \autoref{sec:motivationa and background lie}, I briefly discussed the \textit{exponential map} as a way to convert between the Lie group and the Lie algebra. Here, we will take that definition and solidify its meaning.
...
Our Lie algebra, in general, is not going to be a number. As we saw with the cross product, it is often a matrix or complex number instead. How can we find an exponential for these?
...
This is the definition that we use for all of the more complex cases, such as complex numbers and matrices. Since we are only using integer indices for $n$, all of these operations are well-defined and valid.
...
We start by squaring both sides of the equality $$A^2=\left(PDP^{-1}\right)^2$$ which we can expand to $$A^2=PDP^{-1}PDP^{-1}$$We notice that the middle term, $P^{-1}P$, reduces to $I$, so the sum is simply $$A^2=P(DID)P^{-1}$$which we can express plainly as $$A^2=PD^2P^{-1}$$
\end{example}
...
\begin{exercise}[Show $e^{tr(A)}=\det\left(e^A\right)$]
    To ensure that you understand this method of computing the matrix exponential, show that $e^{tr(A)}=\det\left(e^A\right)$ for the matrix: $$A=\begin{bmatrix}
        4&1\\2&5
    \end{bmatrix}$$
    Bonus points are awarded if you can show this property holds for a general diagonalizable matrix $A$!
\end{exercise}


\section{Rotations}
Volume II has extensive information on rotations, but it will be helpful to solidify that information with the mathematics of Lie groups. Additionally, this means that we can gain an understanding of \textit{any possible} implementation of rotation schemes, because this underlying principle dictates all of them.
...
\begin{enumerate}
 ...
 \begin{equation}
    ...
    \end{equation}
Which can be simplifed using the identity that $(R\vec{v})^T=\vec{v}^TR^T$ to:
 \begin{equation}
        \vec{v}^T \vec{w}=\vec{v}^TR^TR\vec{w}
    \end{equation}
For this statement to be true $R^TR$ must be equal to the identity matrix, $I$. The only way this can occur is if:
\begin{enumerate}
...
\end{enumerate}
As mentioned in the introduction to Lie theory, the Lie algebra represents the tangent space of the Lie group. We can intuitively think of the tangent space as taking a derivative. So, we can create our mathematical description of the Lie algebra for rotations in this way.
...
$$R=I+\varepsilon A$$
The matrix $A$ is representative of the Lie Algebra, the form of which we have yet to define. We can use this relationship in \eqautoref{eq:rotationDef} to find properties of A:
...
\end{gather}
This relationship is called \textit{skew symmetry}, and it is the fundamental property of infinitesimal rotations (or the Lie algebras of rotations, put another way). One notable feature of skew symmetric matrices is that the diagonal must be zero for the property to hold. 

What about the condition on the determinant
We call the Lie algebra for which this condition holds $\mathfrak{so}(n)$.


Skew Symmetric - Lie group

exponentiate to obtain the lie algebra

One of the simplest cases in showcasing the relationship between the Lie group and the Lie algebra is the rotation.
...
We can use the exponential map to go from the Lie group to the Lie algebra. For matrices, we use the matrix exponential as the exponential map. Performing the exponentiation, we have that $e^{A}$ is a rotation matrix.
...
MATLAB has no built in functions for variational or \glspl{symplectic int}, so to implement them we need to discuss their properties in full. Our discussion will build off the mathematical discussion of the $SL(n)$ group, so a thorough reading of \autoref{chap:lie theory} is recommended.

We will use a group called the 
...
I may, at some point, devote an entire book to the study of modern physics and the application of variational calculus and energy methods there. Since that is yet to be released, I choose to devote this chapter to some of the applications of these mechanics in the more broad realm of physics outside of the studies in aerospace. This chapter will focus on high level overviews rather than in-depth discussion of mathematics. Some sufficient mathematics has already been detailed in \autoref{chap:lie theory}, so I will not go to great depths in this chapter.
...
Control Theory, at its base, is about understanding how a system reacts to inputs and how we can change those inputs to modify the system. We want to take a system and bring it to a desired `state'. To start, we need to define what we mean by a `system' in the context of control theory.

\subsection{Systems}
In control theory, a system 

\chapter{The Laplace Domain}

We start our exploration of control theory by exploring the \gls{laplace transform}
\textit{}
The \gls{laplace transform} is an integral transform, much like the \textit{Fourier Transform}. We find the Laplace transform of special use for its applications in solving linear differential equations.
...
\Glspl{eigenfunction} are in essense, the building blocks of the solutions for equations. When we have a linear operator, such as an integral or derivative, there must exist a function that when acted upon by this operator, is unchanged by the operator aside from being scaled. This is precisely what we mean when we define something as an \gls{eigenfunction}. I find the etyomology of this word especially useful in this case. Eigen, in German, is a word meaning 'self' or 'own'. So, any time we see something, such as an eigenvalue or \gls{eigenfunction}, it is a quantity for which it in some way produces itself. We define the eigenfunction as follows:
$$Df=\lambda f$$
