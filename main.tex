...
The next thing that we need to consider in our journey 
...
\begin{example}[The Free Particle]
    The free particle likely has the simplest possible \gls{Lagrangian}. Since there are no potentials involved, we can fully describe the \gls{Lagrangian} of the free particle with $L=T$. Since the object is a point mass particle, the \gls{Lagrangian} is thus $$L=\frac{1}{2}m\dot{x}^2$$
...
We can notice that the expression for the \gls{Hamiltonian} still has $\dot{q}_j$ in it, so we still need an expression for the generalized velocities. However, we no longer need to take the derivative with respect to this term, so we have reduced the order of the differential equation.

\section{The Hamiltonian as Total Energy}
In the introduction to this section, I mentioned that the \gls{Hamiltonian} has an interpretation as the total energy of the system. This is not always the case, but we can show when this will be true.
...
\begin{example}[Matrix Differential Equations]
    ...
    $$\dot{x}=Ax$$
    Where $A$ is an $n{\times}n$ matrix with constant coefficents and $x$ and $\dot{x}$ are $n{\times}1$ vectors. We can find the solution of this equation through separation and integration. We start by performing the seperation of variables:
    $$\frac{dx}{x}=Adt$$
...
It turns out that these are the only two conditions that we need to satisfy for controllability of a linear system.
...
At the end of this chapter, we will apply this controller to a real inverted pendulum system that I have built. My hope is that this will help demonstrate some of the practical parts of the implementation into a real world system.

\begin{figure}[ht]
    \centering
    %\includegraphics[width=\linewidth]{Images/Inverted Pendulum.pdf}
...
%Straight Lines [id:da7877170508964265] 
\draw    (270,70) -- (314.27,199.55) ;
\draw [shift={(314.27,199.55)}, rotate = 71.13] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]      (0, 0) circle [x radius= 4.69, y radius= 4.69]   ;
\draw [shift={(270,70)}, rotate = 71.13] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]      (0, 0) circle [x radius= 4.69, y radius= 4.69]   ;
%Straight Lines [id:da5998055377665403] 
...
% Text Node
\draw (252,52.4) node [anchor=north] [inner sep=0.75pt]    {$m$};
% Text Node
\draw (261,132.4) node [anchor=north west][inner sep=0.75pt]  {$l$};
...
\begin{itemize}
    ...
    \item Friction and damping in the system is neglected
...
We can identify that the term  $\frac{1}{2}ml^2$ can be replaced by the moment of inertia, $I$. Finally, summing the two values of $i$ gives us the following kinetic energy for the system:
...
Linearizing the system is the next apparent step, as this will aid us in building a linear system. First, we must find the fixed points of the system. To find the fixed points of the system, we consider values of $x$ and $\theta$ such that the first derivatives are zero at that point. We also consider $F=0$, since we can produce an arbitrary $F$ to stabilize the system (as we will later do). It is for that reason we must consider $F=0$ when considering stable points of the system.

Since $x$ is not directly present in either equation, we are free to select any value of $x$ to produce a stable system. The physical interpretation is that the system can be stable for any horizontal position, which satisfies our intuition about the system. Considering $\theta$, 

since all of the non-linear terms are trigonometric. First, we can find the fixed points of the system.
