...
void rt_OneStep(void)
{
   ...
   */
  PID_Controller_SetEventsForThisBaseStep(eventFlags);
  ...
  currentTime = (extmodeSimulationTime_T) (((PID_Controller_M->Timing.clockTick0
    +PID_Controller_M->Timing.clockTickH0* 4294967296.0) * 1) + 0)
    ;
  PID_Controller_step0();
     ...
     case 1 :
      currentTime = (extmodeSimulationTime_T)
        (((PID_Controller_M->Timing.clockTick1+
           PID_Controller_M->Timing.clockTickH1* 4294967296.0) * 2) + 0)
        ;
      PID_Controller_step1();
...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(PID_Controller_M, 0);
  ...
  errorCode = extmodeInit(PID_Controller_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (PID_Controller_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(PID_Controller_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(PID_Controller_M);
