...
\begin{exercise}
    ...
    Find the Laplace transforms of the following functions (you may use the Laplace tables provided in \cite{dawkins_differential_nodate}).
    \begin{enumerate}
        \item $t\sin 4t$
        \item $e^{-t}\cos3t$
        \item $\sinh{3t}$
...
In practice, we don't solve for the inverse Laplace transform using this contour integral when solving by hand. Instead, we try to make the equation amenable to being represented as one of the known \glspl{laplace transform}. These methods involve algebraic manipulation to reach one of the known forms of the Laplace transform.
...
With that, we have developed the appropriate background with the \gls{laplace transform} to begin using this formula in control theory.
...
\begin{example}[Linear system example]
...
Although this state space representation is useful, what we will actually start with is the \gls{transfer function} representation. As it turns out, anything we can represent with a \gls{transfer function} must also be linear (and also time invariant in the cases we study). Remember, our Laplace transform is only valid for linear differential equations, so anything we represent in that way will automatically satisfy linearity. 
...
As an example, if we want to go from an input $R(s)$ to an output $Y(s)$, we can use an intermediate \gls{transfer function} $G(s)$, so that the relationship is:
$$R(s)G(s)=Y(s)$$
...
With that, we have all the principle knowledge to create a transfer function from a block diagram, or to construct one from a differential equation using the \gls{laplace transform}. Our objective now is to understand how we may use these transfer functions to understand the stability of a system.
...
\end{figure}
Our resulting stability criteria is this: If all poles of the transfer function are in the left half of the complex plane, then the transfer function is stable.
...
\end{exercise}
As we say in the last exercise, the Routh stability criterion has the interesting property that it allows us to determine whether a system will be stable under certain conditions. If we have direct control other that value, $k$, we may alter the stability of the system. Choosing appropriate values of $k$ is one good way that we may systematically construct a controller!
...
Now that we have explored the use of the pole-zero plot for stability, we may use this to actually design a controller. Designing with the pole-zero plot is known as root locus design. We can design a number of controllers with this method, including PID controllers and lead-lag compensators.
