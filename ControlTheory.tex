...
Next, I choose to discuss some aspects of optimal estimation and control. This has deep ties to our discussion of the calculus of variations. This section will include some discussion of Kalman filtering and some parts of optimal control in aerospace applications, such as LQR controllers. 
...
As we can see, this definition is quite broad. So, we need to narrow it down and focus on the types of systems that we want to analyze here. We can classify systems into several main categories for futher analysis.
...
\end{enumerate}
The operations above are the only operations we can perform, because any other type of input will violate the zero-input zero-output quality of the transfer function.

We can see that applying any of these operations will yield another sine wave, with the only possible variation being the phase and amplitude of the wave!

\begin{exercise}
    Show that all four identities must return a sine wave as a result. You may need to use some trigonometric identities to prove the addition of sine waves.
\end{exercise}

We call the change in the amplitude of the input sine wave the gain, and the change in the phase is simply the phase. The convention is that the a negative phase

So, with these tools, we can compute the gain and phase that result from inputting a sine wave into a transfer function.
...
Our previous discussions on the best fit linear model and other filters have set us up to finally discuss the Kalman Filter. The Kalman Filter is a linear filter which provides the best possible estimate for a linear system.
...
It is important to note that there is a subtle distinction between a consistent and an unbiased estimator. A consistent estimator may converge at $t\rightarrow\infty$ to the true value of $\textbf{x}$, but at any time prior its estimate does not agree with the true value. This can be seen quite evidently in 

\begin{figure}
    \centering
    \includesvg[width=0.75\linewidth]{}
    \caption{Caption}
...
Because some of the theory in this chapter is quite dense, I have restructured this chapter slightly. An intuitive explanation of optimal controllers is discussed first, alongside the MATLAB implementation, and then the theory is discussed in the later portion of the chapter. I have done this so that the reader can more intuitively grasp some of the reasons why the mathematical formulation is structured in the way that it is.

\subsection{LQR Control}
\gls{lqr} control is a type of control which minimizes a quadratic cost function. 
...
We can alternatively represent this minimization problem as:
$$J^*=\min$$
...
\section{LQR Control}
For the pendulum, an \gls{lqr} controller is chosen. Since the system has full state feedback, the \gls{lqr} controller works well for the control system 
...
Generally, stepper motors and servo motors have very good positional control while DC motors have very good speed control. There are overlaps between the technologies, especially as stepper and servo motors become more common and better methods for driving them are developed. 
\subsection{}
