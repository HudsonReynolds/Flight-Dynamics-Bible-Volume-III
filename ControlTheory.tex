...
An introduction to control theory as a whole is essential to understanding aerospace applications. This section will cover \gls{lti} systems and the usage of the \gls{laplace transform} and transfer functions in analyzing these systems. Linear systems are quite simple to model mathematically because they exhibit useful properties such as superposition. For this reason, linear models are studied quite extensively.

Next, I choose to discuss some aspects of optimal estimation and control. This has deep ties to our discussion of the calculus of variations. This section will include some discussion of Kalman filtering and some parts of optimal control in aerospace applications, such as \gls{lqr} controllers. 

Of course, nearly all systems are non-linear to some degree. Thus, it is prudent to cover some examples where we need to model non-linear systems. Non-linear systems will be discussed in brief at the end of the chapter. Non-linear systems, in general, are hard to generalize mathematically, so I take the approach of performing linear analysis for the system. Then, we use simulation to fine tune the linear model. Non-linear dynamics are easily simulated in MATLAB, which we have shown extensively in Volume I and Volume II. This is the common method by which non-linear systems are dealt with.
...
Control theory, at the fundamental level, is about understanding how a system reacts to inputs and outputs. Because this definition is so broad, we can apply control theory to a vast number of things -- even many that you may not normally think of as control theory. Classic examples of control theory include the cruise control in a car or the moving control surfaces of an aircraft to change its trajectory. However, even mundane things are control systems. The knob on your toaster than dictates how long it cooks is a control system!

As we can see, this definition is quite broad. So, we need to narrow it down and focus on the types of systems that we want to analyze here. We can classify systems into several main categories for further analysis.
...
One relationship of transfer functions that we have not mentioned yet is that when a sinusoid is input into the transfer function, the output at steady state must also be a sinusoid of the same frequency!
\begin{figure}[H]
    \centering
...
There are other methods by which the state estimation can be achieved that are not related to the Kalman filter
...
The main reason to do so is because we want to control both the position and the angle of the pendulum. Since our track has a limited length, we need to force the $x$ position back towards 0. This is perfect for the \gls{lqr} controller, which is naturally a \gls{mimo} controller. If we were to use \gls{pid} for the control, we would need two seperate controllers, one which controls the angle, and another which controls the position. The fact that \gls{pid} is \gls{siso} only is one of the big drawbacks of \gls{pid} control scheme!

The selection of the \gls{lqr} controller does introduce some additional design criteria. First and foremost, the \gls{lqr} controller requires that the system has full state feedback.
...
In cross section A-A, the rotary encoder can be seen. The rotary encoder is one of the most imporant components in the design as the angle measurement of the arm is very important in understanding the dynamics of the system. The particular model used here can be found at \cite{noauthor_yuecoom_nodate}.
...
\section{Motor Control Algorithms}
...
For DC motor control, \gls{pid} controllers are quite common. Other control algorithms may be implemented, such as \gls{lqr}, but their implementation has been discussed elsewhere in this text quite thoroughly. \Gls{pid} control is quite popular in such applications for it's widespread use, easy tunability, and simple application in the time-domain. To implement a \gls{pid} loop for a real world system, there are several factors that must be considered:
\begin{enumerate}
    \item Sensor noise and data dropouts
    \item Motor modeling
    \item \gls{pid} gains
\end{enumerate}

Here, I will detail the methods of implementation that have been done for this system. More than anything, this is to give a taste of what can be done for such a system, and is not representative of the full scope of choices that can be made. Each control system will have its own challenges that must be overcome! 
...
\begin{tikzpicture}
	...
	\node[shape=rectangle, draw, line width=1pt, inner sep=0, minimum width=0.318cm, minimum height=0.288cm] at (6, 2.93){};
	\draw (1, 4) to[american voltage source] (1, 1);
	\draw (1, 4) to[cute inductor] (6, 4);
	\node[shape=rectangle, draw, line width=1pt, inner sep=0, minimum width=0.318cm, minimum height=0.288cm] at (6, 2.07){};
	\node[shape=ellipse, fill={rgb,255:red,255;green,255;blue,255}, draw, line width=1pt, inner sep=0, minimum width=0.825cm, minimum height=0.825cm] at (6, 2.5){};
	\draw (6, 4) -| (6, 3.092);
	\draw (6, 1.908) -| (6, 1);
	\draw (6, 1) |- (1, 1);
\end{tikzpicture}
...
The DC motor operates through the generation of a magnetic field and a current carrying conductor in the motor. The interaction of these magnetic fields and electric currents induces a torque:
$$\tau=Ki\phi$$
