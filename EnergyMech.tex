...
Fermat is famous in mathematics for \textit{Fermat's Last Theorem}, which remained unsolved for 358 years. A solution involving elliptic curves exists, on which you can find an in-depth discussion in \cite{hellegouarch_invitation_2001} (this is entirely irrelevant to our work here). 
...
If you want to gain a better understanding of this relationship, an interactive MATLAB script "Mirror.m" in the GitHub. This will run a simulation of this scenario. In this toy model it can be seen how the vectors add in a similar way to our diagramatic depiction. For a true physical system, these effects occur on a much smaller scale. However, we will still see the same effects arise and the form spiral patterns near the ends, where the vectors tend to cancel each other out. Together, they form a very beautiful spiral that resemble the sound holes of a string instrument, which we can see in \autoref{fig:mirror vec 2}. Again, only the components near the path of least time add together! So, on the large scale, we say that light does indeed take the path of least time, even though we can show on the quantum scale that it is free to do anything!
...
\end{figure}
Let's extend the analogy between single variable calculus a little bit further. In standard calculus, when we do a small variance $dx$, we get some change $dy$. At a maximum or minimum, $dy$ will be zero when we do a shift by $dx$. By the same reasoning, when using a whole function, we know we must have reached a local minimum or maximum whenever we add some $\varepsilon\eta(x)$ and the value of our whole function doesn't change.\endnote{You may wonder at this point what it means to have the `value' of a function. We will return to this point later.}

Because so many physical phenomena are governed by this behavior to minimize certain quantities, the calculus of variations became a prominent field of study from the 17th through the 20th centuries. Of course, the calculus of variations also has many applications outside of the confines of physics, some of which will be briefly explored at the end of this chapter. The key component of this theory that we need for energy mechanics is called the Euler-Lagrange equation, which we will now derive.
...
You may notice that these conditions are themselves a function of $x$. This is where we see additional complexity as compared to single variable calculus, as we must minimize over this entire range of values.\endnote{In fact, we must minimize our function over a range that has an infinite number of values. For this reason, the calculus of variations is sometimes cited as a type of infinite dimensional calculus.} We call the function that we are trying to minimize $f$. 

To reiterate, the conditions take the form of a function, as they are dependent on some variable $x$. We need to find another function, $f$, which abides by these constraint equations. Since $f$ has a dependence on these constraints, we may write $f$ as:
$$f[y(x),y'(x);x]$$
...
Since our function exists over a whole range of values of $x$, the sensible thing to do is to consider the function $f$ over all values of $x$, and assign a value to it that way. Of course, we can do this quite easily with an integral! We can evaluate this at two set endpoints, $x_1$ and $x_2$, and this will return a single number value that represents the function. We call this scalar output a \gls{functional}. This \gls{functional} is essentially a function of $f$, so it is a function of a function. We typically describe a functional with the letter $J$, and we write the form of the integral as:
...
This idea of a functional is one of the key tools in the calculus of variations. For our purposes, we only want to use this property to solve for an optimal function $f$. More precisely, we are attempting to find conditions on the function, $y(x)$ and $y'(x)$, which yield an extrema of $J$. 
...
Sometimes, we see a delta notation used for the calculus of variations. It is useful to have a shorthand for these partial derivative quantities. For a quantity $\chi$, the variation, $\delta$ is given as:
$$\delta\chi\equiv\frac{\partial\chi}{\partial \varepsilon}d\varepsilon$$
Let's think briefly about what this says. It reads `a variation of $\chi$ is given by the partial of $\chi$ in the $\varepsilon$ component, multiplied by some small variation $d\varepsilon$'. This essentially says that a variation of $\chi$ is some small nudge by $\varepsilon$. Using this notation, we can rewrite \eqautoref{eq:diff functional final} after some algebraic manipulation as:
\begin{equation}
...
\begin{enumerate}
    ...
    \item Rearrange our equation to be amenable to the \gls{Euler Equation} (write our function in terms of $y(x), y'(x), x$).
...
We recall that in Newtonian mechanics, we describe reference frames with which we describe the positions, velocities, and accelerations for an object. We represent the position, velocity, and acceleration of a particle in terms of these coordinate systems as a tridimensional vector. 
...
\end{gather}
The quantity $\mathcal{M}$ is the number of \gls{dof}. However, this is the number of \gls{dof} for the whole system, which is why it can exceed 6 in many cases!
...
\color{blue}\boxed{\color{black}\frac{\partial L}{\partial q_j}=\frac{d}{dt}\frac{\partial L}{\partial \dot{q}_j},\quad j=1,2,\cdots M}\\
\mathrm{\color{blue}Generalized \ Euler-Lagrange \ Equations}
\end{gather}
Notice the similarity of this equation to Newton's 2nd law, $F=\frac{dp}{dt}$. These two equations are so similar in fact, that we can give names to each of the terms. On the left, we have something which looks like the force:
$$\frac{\partial L}{\partial q_i}=i\text{th }\text{generalized force}$$
and on the right, we have what looks like the time derivative of the momentum, so:
$$\frac{\partial L}{\partial\dot{q}_i}=i\text{th generalized momentum}$$
We can think of these terms as \textit{analogous} to the ones in Newton's 2nd law, but not always precisely the same. Before we move on a few more ideas, let's go through a full example to show this \gls{Lagrangian} approach.
...
With \gls{Lagrangian} mechanics, we can employ several methods to understand conservation. These ideas about conservation ultimately stem from what we call \textit{Noether's Theorem}.
...
Before we move on the mathematical formulation and proof of Noether's Theorem, it would be prudent to consider a few more key points. Firstly, we should clarify a key point: A certain system may or may not conserve a quantity, but the universe as a whole will conserve that quantity. That is to say, the conservation of energy, momentum, charge, etc. is a fact of the universe\endnote{For the pedants, in an expanding universe the total energy is indeed increasing. Lowly aerospace engineers need not worry about this.}; for any given system these quantities are not guaranteed to be conserved.
...
\end{align}
In classical mechanics, these invariances are the ones that we see most often. However, there are more symmetries that occur, and they are very fundamental in our understanding of the universe and how forces arise. To understand these symmetries requires some physics beyond the scope of this volume.\footnote{See \cite{zee_quantum_2025} for a good introductory text explaining some of these symmetries in quantum field theories.}
...
When we have a continuous symmetry between these two, there is no difference in the action! So $S[\gamma]=S[\gamma']$. Additionally, this transformation $T$, must be infinitesimal. 
...
The \gls{Hamiltonian}, on the other hand, is constructed as $H=T+V$. This is something we can recognize! It's the total energy of the system. So we can get a more intuitive feel for the system using the Hamiltonian. We take the conservation of total energy to be a fact of physics, so it's a good definition to have in our formulation. ,Energy is simply changing form and that attribute allows us to understand the trajectory of the system. Something about that description sounds a bit more `fundamental' than the \gls{Lagrangian}.
...
This formulation may be conceptually unsatisfying in some way. Consider that we may only observe the state of a system at one point in time. We may know all of the values of $q_i$ by looking at the system. To determine it's derivative, however, we would need to look at some $dt$ in the future to determine the evolution of those $q_i$'s -- and thus the $\dot{q}_i$'s. This time difference is infinitesimal, but it still means that we cannot determine the state of the system \textit{precisely} from an observation of a single time. To solve this, we want to introduce something into our formulation that is more fundamental that $\dot{q_i}$. Instead of a generalized velocity, we would like something which is more like a generalized momentum. If you wonder why the momentum would be more general, consider that $F=ma$ may be written more precisely as $F=\frac{d\vec{p}}{dt}$. Knowing velocity alone does not give us direct information about the forces, but knowing the momentum as a whole \textit{does}.

With all that being said, we would like to develop a mathematical strategy which can alleviate these two issues. What if we could transform one of these coordinates, $\dot{q}_i$ so that it does not require a derivative? Doing so, we would resolve both issues. Luckily, we can do this! We'll use something called the \textit{Legendre Transform} to do so.
...
The way the Legendre transform accomplishes this is actually quite simple -- it takes information about the derivative and turns that into the value of a function. This property is precisely what allows us to go from a generalized velocity into a momentum.

It turns out that derivative of a function encodes a lot of information about the function. In fact, given just the derivative of a function, we can fully reconstruct the original function through integration. This is exactly what we do with numerical integration schemes, which you have read about in Volume I. The only thing we are missing is some initial condition.
...
\begin{gather}\label{eq:legendre}
    ...
    \color{blue}\text{Legendre Transform}
\end{gather}
...
What we hope to accomplish is to capture all of these in a single formula for momentum. As a result, we should define a \textit{generalized momentum}, analogous to what we have done for generalized positions and velocities. Of course, this notion of the \gls{gen momentum} is more flexible than the ordinary momentum. To define the \gls{gen momentum}, we can consider the example of a free particle -- that is a particle acted upon by no potentials or forces.
...
As it turns out, the way that we calculated the momentum in the example, as $\frac{\partial L}{\partial \dot{x}}$, is what we define to be the \gls{gen momentum}. We can formalize this for any generalized velocity, as
\begin{gather}
    \color{blue}\boxed{\color{black}p_j=\frac{\partial L}{\partial\dot{q}_j}}\\
    \color{blue}\mathrm{Generalized\ Momentum}
\end{gather}
To make sense of this definition, let's go back to our \gls{Lagrangian} momentarily. We remember that our equation there is:
$$\frac{\partial L}{\partial q_j}=\frac{d}{dt}\frac{\partial L}{\partial\dot{q}_j}$$
So, we can reformulate this to mean
$$\text{Generalized Force}=\frac{d}{dt}p_j$$
In other words, a generalized force is the time rate change of a generalized momentum. This should, of course, remind you of Newton's 2nd law.

This definition of \gls{gen momentum} is much more powerful than the standard definition that we use. This definition, in fact, provides an answer for all of the problems that we listed before. By using the definition with the \gls{Lagrangian}, we are able to solve problems in quantum mechanics, special relativity, and of course, classical mechanics!
